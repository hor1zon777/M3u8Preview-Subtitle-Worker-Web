...
def main():
    ...
    device = 'cpu' if a.no_gpu else 'auto'
    compute_type = 'int8' if a.no_gpu else 'float16'
    ...
    segments, info = model.transcribe(
        a.wav,
        language=None if a.lang in ('', 'auto') else a.lang,
    ...
    # 与 whisper-cli 行为对齐：SRT 为空的 → exit 非 0
    if os.path.getsize(srt_path) == 0:
        print("ERROR: whisper output SRT is empty", file=sys.stderr, flush=True)
        sys.exit(1)
